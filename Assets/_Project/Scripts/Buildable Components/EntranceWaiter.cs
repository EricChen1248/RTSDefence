    ...
    public class EntranceWaiter : MonoBehaviour
    {
        ...
        private IEnumerator WaitAndEnter(Component other)
        {
            ...
            var waitPos = transform.position +
                          ((other.transform.position - transform.position + transform.forward).sqrMagnitude >
                           (other.transform.position - transform.position - transform.forward).sqrMagnitude
                              ? transform.forward
            ...
            while (agent.remainingDistance > 0.5f) yield return null;

            for (var i = 0; i < WaitTime; i++) yield return new WaitForFixedUpdate();
